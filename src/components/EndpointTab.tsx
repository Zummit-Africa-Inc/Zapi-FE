...
const EndpointTab: React.FC<Props> = ({id}) => {
    const { inputs, bind, select, handleClear } = useFormInputs(initialState);
    ...
    const toggleExtraParams = () => setIsParamsOpen(prev => !prev)
            ...
            {isAdding && (
                <form onSubmit={handleSubmit}>
                    <Stack direction="row" alignItems="center" spacing={4} mt={4} mb={2}>
                        ...
                        <IconButton onClick={toggleExtraParams}>
                            {isParamsOpen ? <Remove /> : <Add />}
                        </IconButton>
                    </Stack>
                    {(isParamsOpen || method === "post") && (
                        <>
                ...
                {isAdding && (isParamsOpen || method === "post") && (
